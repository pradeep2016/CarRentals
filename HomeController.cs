...
namespace CarRental.Controllers
{
    public class HomeController : Controller
    {
        ...
        public ActionResult Index(FormState state)
        {
            ...
            var clientCars = cars.Select(c => new { c.id, c.Brand, c.Count, c.Photo, c.Price, c.TypeId, key = c.id, label = c.Brand });
            var list = new List<CarChildModel>();

            _ConfigureLightbox(scheduler, clientCars);

            //load cars to the timeline view
            _ConfigureViews(scheduler, clientCars);
        ...
        private List<SelectListItem> _CreatePriceSelect(DHXScheduler scheduler, string selected)
        {
            ...
            {
                var item = new SelectListItem() { Value = pr, Text = string.Format("Rs {0}", pr) };
                if (pr == selected)
                    item.Selected = true;
                prices.Add(item);
            }
